...
Tidy data of monthly river or lake levels information from the
DLY_LEVELS HYDAT table. `station_number` and `prov_terr_state_loc` can
both be supplied. If both are omitted all values from the `hy_stations`
table are returned. That is a large vector for `hy_monthly_levels`.
