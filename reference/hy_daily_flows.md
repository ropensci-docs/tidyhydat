...
Provides wrapper to turn the DLY_FLOWS table in HYDAT (or historical web
service) into a tidy data frame of daily flows. `station_number` and
`prov_terr_state_loc` can both be supplied. If both are omitted all
values from the `hy_stations` table are returned. That is a large tibble
for `hy_daily_flows`.
...
- **FALSE**: Forces use of historical web service (requires `start_date`
  and `end_date`)
