...
Tidy data of monthly loads information from the SED_DLY_LOADS HYDAT
table. `station_number` and `prov_terr_state_loc` can both be supplied.
If both are omitted all values from the `hy_stations` table are
returned. That is a large vector for `hy_sed_monthly_loads`.
...
- **Date_occurred**: Observation date. Formatted as a Date class. MEAN
  is a annual summary and therefore has an NA value for Date.
